...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0, 
        ...
        self.alpha = alpha 
        self.gamma = gamma
        self.no_weight_vfl_epoch = -1
        self.epoch = -1
    
        assert self.cost_class != 0 or self.cost_bbox != 0 or self.cost_giou != 0, "all costs cant be 0"
  
    def _use_weighted_focal_cost(self, epoch):
        # NOTE: this gate is currently NOT applied anywhere in forward(). The matching cost
        # below always uses the weighted focal form (self.alpha / self.gamma), at every
        # epoch, so `no_weight_vfl_epoch` changes the loss (see DEIMCriterion.
        # _get_vfl_weight_warmup_factor) but not the matching. Behaviour is kept as-is
        # because the reported results were produced with it; see
        # tests/engine/deim/test_matcher.py for the xfail that records this.
        return self.no_weight_vfl_epoch == -1 or epoch >= self.no_weight_vfl_epoch
