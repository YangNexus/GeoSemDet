...
def is_multimodal_model(model: Any) -> bool:
    """Detect models that expect multimodal dict input (e.g. DEIMGraph with Get*ModalityTensor)."""
    if model is None:  
        return False
  
    core_model = _unwrap_model(model)
    decoder = getattr(core_model, "decoder", None)
    if getattr(decoder, "supports_depth_prior", False):
        return True
