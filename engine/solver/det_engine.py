...
def _scalar_srg_stats(outputs):
    if not isinstance(outputs, dict):
        return {}
    stats = outputs.get("srg_stats", {})
...
def train_one_epoch(self_lr_scheduler, lr_scheduler, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets) in pbar:
        ...
        with dt[4]:
            ...
            srg_stats = _scalar_srg_stats(outputs)
            if srg_stats:
                metric_logger.update(**srg_stats)
            ...
            if writer and dist_utils.is_main_process() and global_step % 10 == 0:
                ...
                for k, v in srg_stats.items():
                    writer.add_scalar(f'SRG/{k}', v, global_step)
