...
class BaseSolver(object): 
    ...
    def load_resume_state(self, path: str):
        ...
        logger.info(f'Lodding Weights From Model-Path:{path} ...')   
        """Load resume"""  
    ...
    def load_tuning_state(self, path: str):  
        ...
        pretrain_state_dict = self._remap_tuning_state_dict_for_current_model(module, pretrain_state_dict)
    ...
    def _remap_tuning_state_dict_for_current_model(self, module: nn.Module, state_dict: Dict[str, torch.Tensor]):
        """Add aliases for loading standard DEIM checkpoints into parsed SRG/DEIMGraph models."""
        target_state = module.state_dict()
        state_dict = remove_module_prefix(state_dict)
        if module.__class__.__name__ != 'GeoSemDet':
            return state_dict
        ...
        def add_alias(src_key: str, dst_key: str):
            nonlocal mapped_count, skipped_count
            if dst_key in remapped:
                return
            if dst_key in target_state and target_state[dst_key].shape == state_dict[src_key].shape:
        ...
        encoder_prefix_map = {
            'encoder.input_proj.0.': 'encoder.0.',
            'encoder.input_proj.1.': 'encoder.1.',
            'encoder.input_proj.2.': 'encoder.2.',
            'encoder.encoder.0.layers.0.': 'encoder.3.',
            'encoder.lateral_convs.0.': 'encoder.4.',
            'encoder.lateral_convs.1.': 'encoder.8.',
            'encoder.fpn_blocks.0.': 'encoder.7.',
            'encoder.fpn_blocks.1.': 'encoder.11.',
            'encoder.downsample_convs.0.0.': 'encoder.12.',
            'encoder.downsample_convs.1.0.': 'encoder.15.',
            'encoder.pan_blocks.0.': 'encoder.14.',
            'encoder.pan_blocks.1.': 'encoder.17.',
        }

        for key in list(state_dict.keys()):
            if key.startswith('backbone.stem.'):
                add_alias(key, key.replace('backbone.stem.', 'backbone.0.', 1))
                continue

            stage_match = re.match(r'^backbone\.stages\.(\d+)\.(.+)$', key)
            if stage_match:
                stage_idx = int(stage_match.group(1)) + 1
                add_alias(key, f'backbone.{stage_idx}.{stage_match.group(2)}')
                continue

            for src_prefix, dst_prefix in encoder_prefix_map.items():
                if key.startswith(src_prefix):
                    add_alias(key, key.replace(src_prefix, dst_prefix, 1))
                    break

        if mapped_count:
            logger.info(
                f'GeoSemDet tuning key remap: added {mapped_count} aliases '
                f'from standard DEIM/SR checkpoint; skipped {skipped_count} shape-mismatched aliases.'
            )
        return remapped
